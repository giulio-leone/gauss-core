...
#[pyfunction]
pub fn middleware_use_logging(handle: u32) -> PyResult<()> {
...
#[pyfunction]
pub fn middleware_use_caching(handle: u32, ttl_ms: u32) -> PyResult<()> {
    let chain = MIDDLEWARE_CHAINS.get_clone(handle)?;
    chain
        .lock()
        .unwrap()
...
#[pyfunction]
pub fn destroy_middleware_chain(handle: u32) -> PyResult<()> {
...
#[pyfunction]
pub fn guardrail_chain_add_pii_detection(handle: u32, action: String) -> PyResult<()> {
...
#[pyfunction]
pub fn guardrail_chain_add_schema(handle: u32, schema_json: String) -> PyResult<()> {
...
#[pyfunction]
pub fn guardrail_chain_list(handle: u32) -> PyResult<Vec<String>> {
    GUARDRAIL_CHAINS.get(handle, |chain| {
        chain.list().into_iter().map(String::from).collect()
    })
}

#[pyfunction]
pub fn destroy_guardrail_chain(handle: u32) -> PyResult<()> {
...
#[pyfunction]
pub fn create_fallback_provider(provider_handles: Vec<u32>) -> PyResult<u32> {
...
pub fn create_circuit_breaker(
    ...
    let inner = get_provider(provider_handle)?;
...
#[pyfunction]
pub fn py_parse_partial_json(text: String) -> Option<String> {
